...
def eval_ligthning():
    ...
    model = LitBpsToImgNetwork.load_from_checkpoint(
        r"F:\Uni\master\Semester 2\ADL4R\project\codebase\tum-adlr-15\bps_to_image_prediction\lightning_logs\version_6\checkpoints\epoch=216-step=15407.ckpt")
    model.eval()
...
def ingame_evaluation():
    ...
    for i in range(1000):
        ...
        if lq.qsize() == 3:
            ...
            img = env.render()
            img = img[2::8, 2::8, :].transpose([1, 0, 2])
            ax[1].imshow(img)
            plt.show()
            continue
